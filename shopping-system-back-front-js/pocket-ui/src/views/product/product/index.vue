          ...
          <div class="ai-messages">
            <div class="message system">
              <div class="message-content">
                <p>您好！我是您的商品管理助手。我可以帮您更新商品价格、管理库存或提供商品管理建议。请问有什么可以帮到您的？</p>
              </div>
            </div>
...
const sendMessage = async () => {
  ...
  try {
    // 创建事件源连接，使用product专用接口
    const eventSource = new EventSource(`/dev-api/aiAgent/ai/aiAgent/product?message=${encodeURIComponent(userMessage)}`);
...
// 模拟一个函数，该函数会由AI助手调用来更新商品价格
// 在实际实现中，这个函数应该调用后端API
const updateProductPrice = async (skuId, newPrice) => {
  console.log(`更新SKU ID ${skuId}的价格为${newPrice}`);
  // 这里应该是实际的API调用
  // 例如: return api.updateSkuPrice(skuId, newPrice);
  
  // 返回模拟成功消息
  return {
    success: true,
    message: `成功更新SKU ID ${skuId}的价格为${newPrice}`
  };
};
</script>
...
.ai-header {
  ...
  align-items: center;
  padding: 18px;
  background: linear-gradient(135deg, #409EFF, #7a50fa);
  color: white;
