<template>
    <div class="app-container">
      ...
      >
        ...
        <el-table-column prop="imageUrl" label="Logo" #default="scope">
  ...
  const getList=(parentId)=>{
    loading.value = true;
    getTreeSelect(parentId).then(response=>{
  ...
  const fetchData = async (row, treeNode, resolve) => {
    // 向后端发送请求获取数据
    const {data} = await getTreeSelect(row.id);
    // 返回数据
    resolve(data)
  }
  ...
  /** 导出按钮操作 */
  const handleExport=()=>{
    proxy.download('product/category/export', {...queryParams.value}, `category_${new Date().getTime()}.xlsx`)
  }
