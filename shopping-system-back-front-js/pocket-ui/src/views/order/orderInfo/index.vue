  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="orderList">
      ...
      <el-table-column label="金额信息" width="160px">
        <template #default="scope">
          <div>总价：¥{{ scope.row.payMoney?.toFixed(2) }}</div>
          <div>商品金额：¥{{ scope.row.totalMoney?.toFixed(2) }}</div>
          <div>运费：¥{{ scope.row.postFee?.toFixed(2) }}</div>
        </template>
      </el-table-column>
    ...
    <el-dialog title="订单详情" v-model="open" width="70%" append-to-body>
      ...
      <el-descriptions title="支付信息" :column="2" border>
        ...
        <el-descriptions-item label="商品总额">¥{{ form.totalMoney?.toFixed(2) }}</el-descriptions-item>
        <el-descriptions-item label="运费">¥{{ form.postFee?.toFixed(2) }}</el-descriptions-item>
        <el-descriptions-item label="实付金额" :span="2">¥{{ form.payMoney?.toFixed(2) }}</el-descriptions-item>
      </el-descriptions>
      ...
      <el-table :data="orderItems" style="width: 100%">
        ...
        <el-table-column label="单价" width="120">
          <template #default="scope">
            <div class="price-display">
              ...
              <div v-if="scope.row.originalPrice" class="original-price">
        ...
        <el-table-column label="小计" width="140">
          <template #default="scope">
            <div class="price-display">
              <div class="current-price">¥{{ calculateItemTotal(scope.row)?.toFixed(2) }}</div>
              <div v-if="scope.row.originalPrice" class="original-price">
                ¥{{ calculateOriginalTotal(scope.row)?.toFixed(2) }}
              </div>
            </div>
          </template>
        </el-table-column>
...
const calculateItemTotal = (item) => {
  return item.skuPrice * item.skuNum;
};

// 计算原价小计 (如果存在)
const calculateOriginalTotal = (item) => {
  if (item.originalPrice) {
    return item.originalPrice * item.skuNum;
  }
  return null;
};
...
function handleDetail(row) {
  ...
  getInfo(orderId).then(response => {
    ...
    if (response.data) {
      ...
      // 直接使用返回数据，适用于数据直接在response中的情况
      form.value = response;
    }
    
    // 为订单详情中的商品列表添加totalSkuPrice和totalOriginalPrice字段
    orderItems.value = [];
    
    // 根据数据结构选择使用orderItemList或orderItemVoList
    const items = form.value.orderItemList || form.value.orderItemVoList || [];
    
    // 处理商品数据，添加计算字段
    orderItems.value = items.map(item => {
      // 如果后端没有提供originalPrice，我们可以设置一个默认值或暂时不显示
      // 这里假设没有originalPrice
      return {
        ...item,
        // 添加原价字段，如果后端没有提供，可以设置为null或根据业务需求设置
        originalPrice: item.originalPrice || null,
        // 添加总价字段
        totalSkuPrice: item.skuPrice * item.skuNum,
        totalOriginalPrice: item.originalPrice ? item.originalPrice * item.skuNum : null
      };
    });
