...
import {getPayWxPayMiniPayAPI, getPayMockAPI} from '@/services/payment'
import PocketCustomerRec from '@/components/PocketCustomerRec.vue';
import type { LogisticItem , OrderLogisticResult} from '@/types/order'
import {deleteMemberOrderAPI, getMemberOrderCancelByIdAPI} from '@/services/order'
...
const getMemberOrderByIdData = async () => {
  const response = await getMemberOrderByIdAPI(query.id)
...
const onOrderPayment=async(orderId:string)=>{
  ...
  uni.navigateTo({
    url: `/pagesOrder/paymentSelection/paymentSelection?orderId=${orderId}`
  })
}
...
const onOrderDeliveryConfirm = () =>{
  ...
  success: async (success) => {
    if (success.confirm) {
      const resonse = await putMemberOrderReceiptByIdAPI(query.id)
      // 更新订单状态
      order.value = resonse.result
    }
  },
})
}
...
const getMemberOrderLogisticsByIdData = async () => {
  const response = await getMemberOrderLogisticsByIdAPI(query.id)
  logisticsList.value = response.result.list
}
...
const onOrderDelete = async () => {
  uni.showModal({
    ...
    success: async (success) => {
      if(success.confirm){
        await deleteMemberOrderAPI({ ids: [Number(query.id)] })
    ...
    <template v-if="order">
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-if="order?.orderState === OrderState.DaiFuKuan">
          ...
          <view class="button" @tap="onOrderPayment(order.orderId)">去支付</view>
        </template>
        <!-- 其他订单状态:展示再次购买按钮 -->
        <template v-else>
          <!-- 订单状态文字 -->
          ...
          <view class="button-group">
            ...
            <view v-if="isDev && order.orderState === OrderState.DaiFaHuo" class="button" @tap="onOrderDelivery"> 模拟发货 </view>
            <!-- 待收货状态：模拟收货,开发期间使用,用于修改订单状态为已收货 -->
            <view v-if="order.orderState === OrderState.DaiShouHuo" class="button"  @tap="onOrderDeliveryConfirm"> 确认收货 </view>
          </view>
        </template>
      </view>
      <!-- 配送状态 -->
        ...
        <view class="item">
          ...
          >
            ...
            <view class="meta">
              ...
              <view class="quantity">x{{item.skuNum}}</view>
            </view>
          </navigator>
          <!-- 待评价状态:展示按钮 -->
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        ...
        <template v-else>
          ...
          <!-- 待收货状态: 展示确认收货 -->
          <view class="button primary" v-if="order.orderState === OrderState.DaiShouHuo" @tap="onOrderDeliveryConfirm"> 确认收货 </view>
          <!-- 待评价状态: 展示去评价 -->
...
.shipment {
  ...
  .item {
    background-image: url(../../static/images/bargain.png);
